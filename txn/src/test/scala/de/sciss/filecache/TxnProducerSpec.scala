...
import scala.concurrent.stm.{TxnExecutor, InTxn}

// TODO
// - hash collisions are _not_ yet tested!
// - rejections      are _not_ yet tested!
class TxnProducerSpec extends fixture.FlatSpec with Matchers {
  final type FixtureParam = (InTxn, File)
  ...
  "Transactional Producer" should "have as advertised" in { tup =>
    val (tx0, f) = tup
    implicit val tx = tx0
    val cfg     = Config[Int, Int]()
    cfg.folder  = f
    val cache   = TxnProducer(cfg)
